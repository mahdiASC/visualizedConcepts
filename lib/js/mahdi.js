let myRandom = function (min, max, flag = false) {
  ...
  // with 1 Array argument and a max number argument, returns an array of randomly sampled items (without repeat if flag false, with repeat if true) of length max

  if (Array.isArray(min)) {
    if (max) {
      return sampleArray(min, max, flag);
    } else {
...
function sampleArray(arr, num, flag) {
  // returns an array of a sampling from the arr
  // length of array returned depends on num
  // if flag is true, the picks will allow for resampling
  let output = [];
  if (flag) {
    for (let i = 0; i < num; i++) {
      output.push(myRandom(arr));
    }
  } else {
    if (num > arr.length) throw new Error(`With no replacement, sampling: ${num} is larger than array length: ${arr.length}`);
    let tempArr = []; //used to store unique indeces
    while (output.length < num) {
      let inx = myRandom(arr.length);
      if (!tempArr.includes(inx)) {
        tempArr.push(inx);
        output.push(arr[inx]);
      }
    }
...
// simple sort function for general use
// some_array.sort(sortNumber);
...
let avgArray = function (array) {
  ...
  if (!Array.isArray(array) || array.some(x => typeof x != "number")) {
    throw new TypeError("All items in array should be numbers");
  }
...
Object.defineProperty(Array.prototype, "sd", {
  enumerable: false, // issue with for...in loop and prototype methods being included with enumeration
  get: function () {
    return stdDev(this);
  }
...
// Removes the first instance of an item from a given array, modifying the input array
// if flag is true, removes all instances
function removeArrayItem(arr, item, flag) {
...
Object.defineProperty(Array.prototype, "remove", {
  enumerable: false, // issue with for...in loop and prototype methods being included with enumeration
  value: function (item, flag) {
    removeArrayItem(this,item, flag);
    return this;
  }
});


// adds property to array returning last item
...
// adds property to array returning first item
