...
class KmeansCenter extends KmeansPoint{
    ...
    reassignCenter(points){
        ...
        let x,y;
        // if array is empty, will reset to center
        if(points.length == 0){
            x = width/2;
            y = height/2;
        }else{
            x = points.map(x=>x.x).avg;
            y = points.map(x=>x.y).avg;
        }
        this.x = x;
        this.y = y;
    }
...
class KmeansRandomCenter extends KmeansCenter{
    ...
    setError(points){
        // returns average error between all of this centroid's points and itself
        this.error = this.myPoints(points).map(x=>Math.pow(x.distance(),2)).avg;
    }
}
...
class Kmeans{
    ...
    */
    ...
    drawIteration(centers){
        // draws the randomized iterated data to the canvas
        if(centers) {
            this.randomCenters = centers;
            this.randomCenters.forEach(x=>x.tagPoints(this.data));    
        }
    ...
    iterateHandler(){
        // limiting number of full attempts
        if(this.attempts>10){
            noLoop();
            this.drawIteration(this.bestSolution());
        }else{
            if(this.randomCenters.every(x=>x.done())){
                console.log(`Attempt: ${this.attempts}`);
                this.randomCenters.forEach(y=>y.setError(this.data));
                this.resetStep();
            }else{
                this.nextStep();
            }
    ...
    bestSolution(){
        // takes array of solutions and returns best (one with least R-squared)
        let best = {};
        this.solutions.forEach(x=>{
            let errSum = x.map(y=>y.error).sum
            if(!best.solution){
                best.solution = x;
                best.errSum = errSum
            }else{
                if(best.errSum > errSum){
                    best.solution = x;
                    best.errSum = errSum
                }
            }
...
function setup(){
    ...
    x = new Kmeans(3);// input is the user's guess
    x.drawData();
    x.resetStep();
    frameRate(60);
}
